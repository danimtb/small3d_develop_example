...
namespace AvoidTheBug3D {
	...
	GameLogic::GameLogic()
	{
		...
		unique_ptr<Image> startScreenTexture( new Image("jafroch/Small3dExample/resources/images/startScreen.png") );
		renderer->generateTexture("startScreen", startScreenTexture->getData(), startScreenTexture->getWidth(), startScreenTexture->getHeight());
		
		unique_ptr<Image> groundTexture( new Image("jafroch/Small3dExample/resources/images/grass.png") );
		renderer->generateTexture("ground", groundTexture->getData(), groundTexture->getWidth(), groundTexture->getHeight());
		
		unique_ptr<Image> skyTexture( new Image("jafroch/Small3dExample/resources/images/sky.png") );
		renderer->generateTexture("sky", skyTexture->getData(), skyTexture->getWidth(), skyTexture->getHeight());
		
		goat = shared_ptr<SceneObject>(new SceneObject("goat", "jafroch/Small3dExample/resources/models/Goat/goatAnim",19, "jafroch/Small3dExample/resources/models/Goat/Goat.png","jafroch/Small3dExample/resources/models/GoatBB/GoatBB.obj"));
		
		bug = shared_ptr<SceneObject>(new SceneObject("bug", "jafroch/Small3dExample/resources/models/Bug/bugAnim",9));
		bug->setColour(0.8f, 0.7f, 0.04f, 1.0f);
		bug->setFrameDelay(2);
		
		bugVerticalSpeed = ROUND_2_DECIMAL(BUG_FLIGHT_HEIGHT / BUG_DIVE_DURATION);
    
		tree = shared_ptr<SceneObject>( new SceneObject("tree", "jafroch/Small3dExample/resources/models/Tree/tree.obj",1, "jafroch/Small3dExample/resources/models/Tree/tree.png", "jafroch/Small3dExample/resources/models/TreeBB/TreeBB.obj"));
		...
		sound->load("jafroch/Small3dExample/resources/sounds/bah.ogg", "bah");
	...
	void GameLogic::moveGoat( const KeyInput &keyInput )
	{
		...
		if(keyInput.left)
		{
			...
			while(goat->collidesWithSceneObject(tree) || goat->collidesWithSceneObject(tree2))
			{
				goatRotation->y += GOAT_ROTATION_SPEED;
			}
  ...
  {
	  if (keyInput.enter || keyInput.one || keyInput.two || keyInput.three)
	  {
  ...
  void GameLogic::process( const KeyInput &keyInput, int &level)
  {
	  switch(gameState)
	  {
		case START_SCREEN:
			processStartScreen(keyInput, level);
			break;
		
		case PLAYING:
			processGame(keyInput, level);
			break;
		default:
			throw Exception("Unrecognised game state");
			break;
	  }
  }
